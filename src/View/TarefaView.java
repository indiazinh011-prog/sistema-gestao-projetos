...
public class TarefaView {
    ...
    public TarefaView(SistemaController controller, Scanner scanner) {
    ...
    private void criarTarefa() {
        ...
        if (projetos.isEmpty()) { System.out.println("\nNenhum projeto cadastrado. Crie um projeto primeiro."); return; }
        System.out.println("\n--- Projetos disponíveis ---");
        for (int i = 0; i < projetos.size(); i++)
            System.out.printf("[%d] %s%n", i, projetos.get(i).getNome());
        System.out.print("Índice do projeto: ");
        int projetoIndex = lerInt();
        List<Colaborador> colaboradores = controller.listarColaboradores();
        if (colaboradores.isEmpty()) { System.out.println("\nNenhum colaborador cadastrado. Cadastre um colaborador primeiro."); return; }
        System.out.println("\n--- Colaboradores disponíveis ---");
        for (Colaborador c : colaboradores)
            System.out.printf("ID: %d | %s | %s%n", c.getId(), c.getNome(), c.getCargo());
        System.out.print("ID do colaborador responsável: ");
        int colaboradorId = lerInt();
        System.out.println("\n--- Dados da Tarefa ---");
        System.out.print("Nome: ");
        String nome = scanner.nextLine().trim();
        System.out.print("Descrição: ");
        String descricao = scanner.nextLine().trim();
        System.out.print("Prazo (ex: 15/11/2025): ");
        String prazo = scanner.nextLine().trim();
        boolean sucesso = controller.criarTarefa(projetoIndex, nome, descricao, prazo, colaboradorId);
    ...
    private void listarTarefas() {
        ...
        if (tarefas.isEmpty()) { System.out.println("\nNenhuma tarefa vinculada a \"" + projeto.getNome() + "\"."); return; }
